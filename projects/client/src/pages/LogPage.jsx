...
export default function Log() {
 ...
 async function getAll() {
  const all = await api.get('/attendancelog/all', {
   params: {
 ...
 async function getFilter(time) {
  const filter = await api.get('/attendancelog/filter', {
   params: {
    user_id: userSelector.id,
    time: time
   }
   ...
   <Box id="boxLogin" w={'390px'} h={'844px'}>
    ...
    >
     ...
     <Flex flexDir={'column'} padding={'15px'} fontSize={'20px'}>
      ...
      <Input type="month" onChange={(e) => getFilter(e.target.value)}></Input>
     </Flex>
    </Box>
    <Flex
     ...
     ? attendance?.data.map((val) => {
        return (
         ...
         >
          ...
          <Flex w={'25%'} justifyContent={'center'}>
           {val.clock_out ? moment(val.clock_out).format('HH:mm') : '-'}
          </Flex>
         </Flex>
        );
       })
